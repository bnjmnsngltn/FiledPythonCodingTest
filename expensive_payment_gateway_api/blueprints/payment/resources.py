...
class ExpensivePaymentGateway(Resource):
    def post(self):
        ...
        if resp == False:
            return {'status': 404, 'gateway': 'ExpensivePaymentGateway', 'message': 'Cannot reach bank API', 'details': args}

        status = resp['status']
        details = resp['details']
        message = resp['message']

        return {'status': status, 'gateway': 'ExpensivePaymentGateway', 'message':message, 'details':details}
