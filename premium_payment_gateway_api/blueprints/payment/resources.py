...
from premium_payment_gateway_api.blueprints.payment.controllers import make_payment
...
class PremiumPaymentGateway(Resource):
    def post(self):
        ...
        if resp == False:
            return {'status': 404, 'gateway': 'PremiumPaymentGateway', 'message': 'Cannot reach bank API', 'details': args}

        status = resp['status']
        details = resp['details']
        message = resp['message']

        return {'status': status, 'gateway': 'PremiumPaymentGateway', 'message':message, 'details':details}
